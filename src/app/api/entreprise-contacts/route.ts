...
export async function POST(request: NextRequest) {
  try {
    const authError = await checkAuth(request);
    if (authError) return authError;
    // ─── Validation Zod centralisée (identifiants, email) ───────────────────
    const parsed = await parseJsonBody(request, contactEntrepriseCreateSchema);
    if (!parsed.success) return parsed.response;
    const { societeId, nom, prenom, fonction, telephone, email } = parsed.data;
